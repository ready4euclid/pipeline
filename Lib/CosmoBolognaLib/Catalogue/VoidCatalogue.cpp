...
cosmobl::catalogue::Catalogue::Catalogue (const VoidAlgorithm algorithm, const Catalogue halo_catalogue, const vector<string> file, const double nSub, const int n_rnd, const string mode, const double rmax, const int cellsize)
{
  ...
  if (algorithm==_LaZeVo_)
  {
    ...
    for (int rndd=1; rndd<=n_rnd; ++rndd) // 'for' cycle - Cycling to the number of realizations 'n_rnd' set up in the 'input_param.ini' file 
    {
      ...
      coutCBL << "Number of unpaired halos: " << rand_cat->nObjects() << endl;
      coutCBL << "Percentage of unpaired halos: " << (float)rand_cat->nObjects()/halo_cat->nObjects() * 100 << "% " << endl;
      cout << endl;
    }
  }
...
//   print(Divergence);
  ...
  cout << endl;
  coutCBL << "Time spent to count the pairs: " << diff      << " seconds " << endl ;
  coutCBL << "Time spent to count the pairs: " << diff/60   << " minutes " << endl ;
  coutCBL << "Time spent to count the pairs: " << diff/3600 << " hours "   << endl ;
  cout << endl;
